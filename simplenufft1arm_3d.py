...
def process(connection, config, metadata, N=None, w=None):
    ...
    if N is None:
        ...
        traj = loadmat("/server/home/pkumar/repos/rtspiral_pypulseq/out_trajectory/" + traj_name)
    ...
    for arm in connection:
        if arm is not None:
            ...
            if (arm.idx.kspace_encode_step_1 == 0) and APPLY_GIRF:
                ...
                # k_pred = np.flip(k_pred[:,:,:], axis=2) # Drop the z
