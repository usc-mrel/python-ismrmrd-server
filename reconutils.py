...
def process_group(group, frames: list, sens: npt.NDArray | None, device, rep, config, metadata, subtract_flag = False, Pref=None):
	xp = device.xp
	print(f"Rep : {rep}")
	with device:
		...
		if subtract_flag and Pref is not None:
			Nframes = Pref.shape[0]
			Pref = xp.asarray(sp.to_device(Pref))
			scale = xp.max(xp.abs(data))/xp.max(xp.abs(Pref[rep % Nframes,:,:,:].squeeze()))
			Pref = Pref * scale
			data -= Pref[rep % Nframes,:,:,:].squeeze()
	...
	meta = ismrmrd.Meta({'DataRole':               'Image',
						 'ImageProcessingHistory': ['FIRE', 'PYTHON', 'spi_proj'],
