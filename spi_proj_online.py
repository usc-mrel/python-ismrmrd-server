...
def get_projection(frames):
	device = frames[0].device
	xp = np if isinstance(frames[0], np.ndarray) else cp
	with device:
		data = xp.zeros(frames[0].shape, dtype=xp.complex128)
		for g in frames:
			data += g
	return data
...
def process(conn: connection.Connection, config, metadata):
	...
	pre_scan = cfg['reconstruction']['pre_scan']
	subtract_flag = cfg['reconstruction']['subtract_flag']
	...
	msize = int(cfg['reconstruction']['fov_oversampling'] * traj['param']['fov'][0,0][0,0] / traj['param']['spatial_resolution'][0,0][0,0])
	...
	w = traj['w_proj']		# There are 2 DCF, this is the projection one.
	
	# ----------------------------------------
	Pref = None
	ref_proj_fn = get_ref_proj_filename(metadata)

	if not pre_scan and subtract_flag:
		if os.path.exists(ref_proj_fn):
			Pref = loadmat(ref_proj_fn)['P']
		else:
			print(f"Reference projection file {ref_proj_fn} not found. Cannot subtract.")
	if pre_scan:
		subtract_flag = False
		Pref = []
		...
		try:
			...
			while True:
				...
				if ((arm.scan_counter) % window_shift) == 0 and ((arm.scan_counter) >= n_arm_per_frame):
					...
					if pre_scan:
						Pref.append(get_projection(frames))

					if save_complex:
						image = reconutils.process_frame_complex(arm, frames, sens, device, rep_counter, cfg, metadata)
					else:
						image = reconutils.process_group(arm, frames, sens, device, rep_counter, cfg, metadata, subtract_flag = subtract_flag, Pref=Pref)
					end = time.perf_counter()
